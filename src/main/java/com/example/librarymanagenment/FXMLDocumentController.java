...
public class FXMLDocumentController implements Initializable {
    ...
    public void login() {
        ...
        try{
            ...
            prepare.setString(1, studentNum.getText());
            prepare.setString(2, pass.getText());
            result = prepare.executeQuery();

            Alert alert;

            if(studentNum.getText().isEmpty() || pass.getText().isEmpty()){
...
//                    FOR DASHBOARD
                    Parent root = FXMLLoader.load(getClass().getResource("dieuKhien.fxml"));
    ...
    public void numbersOnly(KeyEvent event){
        ...
        if(event.getCharacter().matches("[^\\e\t\r\\d+$]")){
            ...
            studentNum.setStyle("-fx-border-color:#e04040");  // nhap sai phan gach chan doi thanh mau do
        }else{
            studentNum.setStyle("-fx-border-color:#fff"); //khong nhap sai phan gach chan doi thanh mau trang
        }
